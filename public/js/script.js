...
//while(sdt.personnes.length - 1 > 0) {
    setTimeout(() => {
        
        console.log('Le premier patient est ' + sdt.personnes[0].nom)
    }, timer += 1000);
    setTimeout(() => {
    ...
    // }, 2000);
    setTimeout(() => {
        console.log(allPatients[i].nom + " lui explique son problème");

    }, timer += 1000);

    setTimeout(() => {
        console.log('Le docteur ' + doc.nom + ' lui diagnostique : ' + allPatients[i].maladie);

    }, timer += 1000);

    setTimeout(() => {
        trait = doc.diagnostique(allPatients[i].maladie)
        console.log('Le docteur lui son indique son traitement qui est : ' + trait + "\nEt lui donne son ordonnace");

    }, timer += 1000);

    setTimeout(() => {
        console.log(allPatients[i].nom + ' remercie le doc et le paye');
        marcus.paye(doc.consulation)
        doc.patientOut();
    }, timer += 1000);

    setTimeout(() => {
            console.log(allPatients[i].nom + " se rend a la pharmacie ");
    marcus.goTo(pharmacie)
    }, timer += 1000);

    setTimeout(() => {
            console.log(allPatients[i].nom + " est arrivé a la pharmacie ");

    }, timer += 1000);

    setTimeout(() => {
            console.log(allPatients[i].nom + " donne l'ordonnace au pharmacien ");
    ...
    setTimeout(() => {
        console.log(allPatients[i].nom+" paye so medocs");
    ...
    setTimeout(() => {
            console.log(allPatients[i].nom + " prends son medocs");
